...
export const flattenObject = <Input extends object>(
  obj: Input,
  parentKey = ''
): FlattenedObject<Input> => {
  ...
  for (const key in obj) {
    ...
    if (obj.hasOwnProperty(key)) {
      ...
      if (typeof obj[key] === 'object' && obj[key] !== null && !Array.isArray(obj[key])) {
        ...
        // If the value is an array of strings and numbers, join them with a comma
        result[newKey] = obj[key].map((item) => `'${item}'`).join(',')
      } else if (Array.isArray(obj[key]) && obj[key].every((item) => typeof item === 'number')) {
        // If the value is an array of strings and numbers, join them with a comma
        result[newKey] = obj[key].map((item) => `${item}`).join(',')
      } else {
